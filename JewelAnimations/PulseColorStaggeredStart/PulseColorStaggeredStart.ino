/*
PulseColorStaggeredStart
By Kelsey Steven

Made to run effects for a single Neopixel Jewel. 
Pulses all LEDs in a single color with a staggered start aniimation.
*/
...
void handleLightOff(int index) {
  if(index > START_INDEX) {
      long intervalForIndex = START_DELAY * index;      
      if (millis() - timer >= intervalForIndex) {
        lightState[index] = 1;
